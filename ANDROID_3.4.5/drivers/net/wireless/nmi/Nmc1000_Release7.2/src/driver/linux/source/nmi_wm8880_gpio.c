...
{
	printk("%s======hi= %d,gpio_reset_ctrl:%d\n",__FUNCTION__,high_or_low,gpio_reset_ctrl);
    if(gpio_reset_ctrl == WMT_PIN_GP1_GPIO14)
...
{
	printk("%s,gpio_reset_ctrl:%d\n",__func__,gpio_reset_ctrl);
	//if(gpio_chipen_ctrl == WMT_PIN_GP62_SUSGPIO1)
		gpio_free(gpio_chipen_ctrl);
	//if(gpio_reset_ctrl == WMT_PIN_GP1_GPIO14){
		printk("free gpio_reset_ctrl:%d\n",gpio_reset_ctrl);
		gpio_free(WMT_PIN_GP1_GPIO14);
	//}
	printk("exit wifi_card_release_gpios\n");
}
