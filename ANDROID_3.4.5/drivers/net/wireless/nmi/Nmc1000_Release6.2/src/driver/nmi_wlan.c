...
static void nmi_wlan_txq_add_to_head(struct txq_entry_t *tqe)
{
	nmi_wlan_dev_t *p = (nmi_wlan_dev_t *)&g_wlan;
...
static void nmi_wlan_cfg_commit(int type)
{
	nmi_wlan_dev_t *p = (nmi_wlan_dev_t *)&g_wlan;
	...
	**/
	...
	/**
		Add to TX queue
	**/
	nmi_wlan_txq_add_cfg_pkt(&cfg->wid_header[0], total_len);
...
static int nmi_wlan_cfg_set(int start, uint32_t wid, uint8_t *buffer, uint32_t buffer_size, int commit)
{
	...
	if (commit) {
		...
		nmi_wlan_cfg_commit(NMI_CFG_SET);		
...
static int nmi_wlan_cfg_get(int start, uint32_t wid, int commit)
{
	...
	if (commit) {
		p->cfg_frame_in_use = 1;
		nmi_wlan_cfg_commit(NMI_CFG_QUERY);
		if(p->os_func.os_wait(p->cfg_wait,CFG_PKTS_TIMEOUT))
		{
