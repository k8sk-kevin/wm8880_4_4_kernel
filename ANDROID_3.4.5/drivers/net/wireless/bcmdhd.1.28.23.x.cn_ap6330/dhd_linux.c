...
//add by toron, diffenert chip can use its own nvram.txt.
int nvram_flag = 0;//1 stand ap6330, 2 stand ap6210, valued in dhd_sdio.c
#define BCMDHD_AP6330_NVRAM_PATH "/system/etc/firmware/nvram_ap6330.txt"
#define BCMDHD_AP6210_NVRAM_PATH "/system/etc/firmware/nvram_ap6210.txt"
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	if (dhd->threads_only)
        ...
        if (nvram_flag == 2)//ap6210
        {       printk("set nvram to ap6210, nvram_flag=%d\n", nvram_flag);
                bcm_strncpy_s(nv_path, sizeof(nv_path), BCMDHD_AP6210_NVRAM_PATH, MOD_PARAM_PATHLEN-1);
        }
        else
        {       printk("set nvram to ap6330, nvram_flag=%d\n", nvram_flag);
