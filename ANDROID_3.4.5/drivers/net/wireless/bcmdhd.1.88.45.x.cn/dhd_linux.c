...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	if (dhd->threads_only)
        ...
        ret = wmt_getsyspara("wmt.wifi.type", buf, &varlen);
        if (!ret)
        {
                if (!strncmp(buf, "ap6476", 6) || !strncmp(buf, "AP6476", 6)){
                        bcm_strncpy_s(nv_path, sizeof(nv_path), "/system/etc/firmware/nvram_ap6476.txt", MOD_PARAM_PATHLEN-1);
                        printk("set nv_path to ap6476.txt\n");
                }
                else if(!strncmp(buf, "ap6210", 6) || !strncmp(buf, "AP6210", 6)){
                        bcm_strncpy_s(nv_path, sizeof(nv_path), "/system/etc/firmware/nvram_ap6210.txt", MOD_PARAM_PATHLEN-1);
                        printk("set nv_path to ap6210.txt\n");
                }
		else if(!strncmp(buf, "ap6493", 6) || !strncmp(buf, "AP6493", 6)){
                        bcm_strncpy_s(nv_path, sizeof(nv_path), "/system/etc/firmware/nvram_ap6493.txt", MOD_PARAM_PATHLEN-1);
			printk("set nv_path to ap6493.txt\n");
		}
                else
                        printk("don't find nv_path\n");
        }
	ret = -1;
...
dhd_net_attach(dhd_pub_t *dhdp, int ifidx)
{
	...
	set_wifi_name("bcmdhd_ap6476_kk.ko");
...
dhd_module_cleanup(void)
{
	...
	gpio_free(15);//add by dulong , free gpio WAKEUP0
	printk("%s: Exit\n", __FUNCTION__);
}
