...
static int bcmsdh_sdmmc_suspend(struct device *pdev)
{
	...
	wmt_gpio_mask_irq(15);//add by dulong, disable interrupt when suspend;
	printk("%s Enter\n", __FUNCTION__);
...
#endif
	printk("%s Enter\n", __FUNCTION__);
	//add by dulong, enable interrupt when resume.
        wmt_gpio_set_irq_type(15, IRQ_TYPE_LEVEL_HIGH);
        wmt_gpio_unmask_irq(15);
