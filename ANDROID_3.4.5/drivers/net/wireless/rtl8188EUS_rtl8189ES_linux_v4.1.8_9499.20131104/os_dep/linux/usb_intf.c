...
static struct wake_lock wmt_rtl_WakeLock;
static int wifi_wakelock = 0;
...

{
	...
	char retval[16] = {0};
    wifi_wakelock = 0;
	ret = wmt_getsyspara("wmt.io.wifi.wakelock", retval, &len);
	if(!ret){
		if(!strcmp(retval,"1"))
           wifi_wakelock = 1; 
	}
    if(wifi_wakelock){
        wake_lock_init(&wmt_rtl_WakeLock, WAKE_LOCK_SUSPEND, "wmt_rtl_WakeLock");
        wake_lock(&wmt_rtl_WakeLock);
    }
}
...
static void __exit rtw_drv_halt(void)
{
    ...
    if(wifi_wakelock){
        wake_unlock(&wmt_rtl_WakeLock);
        wake_lock_destroy(&wmt_rtl_WakeLock);
    }
